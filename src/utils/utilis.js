...
/** Clear sticky AB flag so plain /signup shows the default (old) flow. */
export function clearAbSignupFlag() {
    if (typeof window === 'undefined') return;
    const isLocalhost = window.location.hostname === 'localhost';
    document.cookie = `absignup=; ${SHARED_COOKIE_EXPIRE}; path=/`;
    if (!isLocalhost) {
        document.cookie = `absignup=; ${SHARED_COOKIE_EXPIRE}; path=/; domain=.msg91.com; SameSite=Lax; Secure`;
    }
    try {
        sessionStorage.removeItem('absignup');
    } catch (_) {}
}
...
export function loginWithGitHubAccount(loginProcess) {
    ...
    if (!loginProcess && isAbSignupActive()) {
        otherParams += `&absignup=a`;
    }
