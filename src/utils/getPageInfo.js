...
export default function getPageInfo(params) {
    ...
    let baseURL =
        pathArray[0] && availableCountries.some((c) => c.shortname.toLowerCase() === pathArray[0].toLowerCase())
            ? pathArray.slice(1).join('/')
            : pathArray.join('/');
    if (params?.slug && params?.slug.length > 1) {
        if (availableCountries.some((c) => c.shortname.toLowerCase() === params?.slug[0].toLowerCase())) {
        ...
        params?.slug &&
        availableCountries?.some((c) => c?.shortname?.toLowerCase() === params?.slug[0]?.toLowerCase())
    ) {
        country = params?.slug[0];
    } else if (params?.slug) {
        page = params?.slug[0];
