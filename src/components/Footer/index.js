...
export default function Footer({ data, pageInfo }) {
    const hidden = data?.hide?.includes(pageInfo?.page);
                ...
                <div className='bg-gray-800 pt-16 pb-10'>
                    <div className='container text-white grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-10  '>
                        <div className='flex flex-col gap-10 w-full'>
                            {data?.products?.links.length > 0 && (
                                <div className='flex flex-col gap-4'>
                                    <span className='font-semibold'>{data?.products?.name}</span>
                            ...
                            {data?.utilities?.links.length > 0 && (
                                <div className='flex flex-col gap-4'>
                                    <span className='font-semibold'>{data?.utilities?.name}</span>
                                    <ul className='flex flex-col gap-2'>
                                        {data?.utilities?.links.map((link, index) => {
                                            return (
                                                <li key={index} className='text-link-white'>
                                                    <Link
                        ...
                        <div className='flex flex-col gap-10 w-full'>
                            {data?.channels?.links.length > 0 && (
                                <div className='flex flex-col gap-4'>
                                    <span className='font-semibold'>{data?.channels?.name}</span>
                                    <ul className='flex flex-col gap-2'>
                                        {data?.channels?.links.map((link, index) => {
                                            return (
                                                <li key={index} className='text-link-white'>
                                                    <Link
                                                        href={getURL('product', link?.link, pageInfo)}
                            ...
                            {data?.resources?.links.length > 0 && (
                                <div className='flex flex-col gap-4'>
                                    <span className='font-semibold'>{data?.resources?.name}</span>
                                    <ul className='flex flex-col gap-2'>
                        ...
                        <div className='flex flex-col gap-10 w-full'>
                            {data?.discover?.links.length > 0 && (
                                <div className='flex flex-col gap-4'>
                                    <span className='font-semibold'>{data?.discover?.name}</span>
                                    <ul className='flex flex-col gap-2'>
                                        {data?.discover?.links.map((link, index) => {
                        ...
                        <div className='flex flex-col gap-10 w-full h-full'>
                            ...
                            {data?.address?.address && (
                                <p className=' text-gray-200 text-end'>{data?.address?.address}</p>
                            )}
                ...
                <div className='bg-gray-900 pt-10 pb-16'>
                    <div className='container text-white flex'>
                        <ul className='flex flex-wrap gap-4'>
                            {data?.policies?.links.length > 0 &&
                                data?.policies?.links.map((link, index) => {
                                    return (
                                        <li key={index} className='flex items-center gap-4'>
                                            ...
                                            {index !== data?.policies?.links.length - 1 && <span>|</span>}
                                        </li>
                                    );
                                })}
                        </ul>
                    ...
                    <div className='container flex flex-col  gap-2 pt-5 text-gray-300 text-sm'>
                        <p>
                            {' '}
                            Copyright 2008-{year}, {data?.address?.name || 'Superheroes, Inc.'} | All rights
                            reserved.
                        </p>
                    </div>
                </div>
