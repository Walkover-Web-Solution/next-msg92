...
import { MdArrowDropDown, MdInfo, MdLogin, MdOutlineCall, MdOutlineLanguage, MdTranslate } from 'react-icons/md';
...
export default function NotificationBarComp({ componentData, pageInfo }) {
    ...
    const showHolidayMessage = useMemo(() => {
        const now = new Date();
        const endDate = new Date(2026, 0, 26, 23, 59, 59); // Jan 26, 2026, 23:59:59
                ...
                <div className='container flex flex-col md:flex-row gap-2 justify-between'>
                    {showHolidayMessage && (
                        <div className='text-sm'>
                            <span className='badge badge-sm badge-warning rounded mt-0.5 mr-1 '>Update</span>
                            <span>
                                Our availability will be limited on <strong>26th January</strong> due to the public
                                holiday.
                            </span>
                        </div>
                    )}
