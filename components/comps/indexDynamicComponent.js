            ...
            <div className="container bg-gray-100 section-y">
                <div className="row">
                    <div className="col-12 col-sm-12 col-md-7 col-lg-7 d-flex flex-column gap-4 align-items-start hero-left">
                        <div className="home-pre-heading c-fs-4 uc">{pageData?.pagetag}</div>
                        ...
                        {pageData?.products &&
                            ...
                            pageData?.products?.content.map((product, index) => {
                                return (
                                    <Link
                                        key={index}
                                        href={`/${path && path + "/"}${product?.name.toLowerCase()}`}
                                        className={` home-product ${product?.name.toLowerCase()}-card  text-dark gap-2`}
                                    >
                                        <div className="d-flex flex-column justify-content-between gap-2">
                                            <img src="./img/icon/hello.svg" className="pro-sec-ico" alt="hello icon" />
                                            <div className="pre-heading c-fs-5">{product?.tag.toUpperCase()}</div>
                                            <div className="pro-heading small-heading">{product?.name}</div>
                                            <div className="product-content c-fs-4">{product?.subheading}</div>
                                            <div className="pro-btn">
                                                {pageData?.products?.buttonname}
                                        ...
                                        <div className="product-img d-flex justify-content-end col-sm-12 col-md-6 col-lg-6 col-xl-6 col-xxl-6 align-items-start">
                                            <img src={`/img/home/${product?.img}`} alt="hello chats" />
                                        </div>
                                    </Link>
                                );
                            })}
                            ...
                            pageData?.channels?.content.map((channel, index) => {
                                return (
                                    <div key={index} className="col-xs-12 col-sm-6 col-md-6 col-lg-4">
                                        <a
                                            href={`/${path && path + "/"}${channel?.name.toLowerCase()}`}
                                        ...
                                        >
                                            <div className="d-flex flex-column gap-3">
                                                <Image
                                                    className="me-2"
                                                    src={`/img/icon/${channel?.name.toLowerCase()}.svg`}
                                                    width={40}
                                                    height={40}
                                                    alt="cannels"
                                                />
                                                ...
                                                <div className="justify-content-between d-flex align-items-center c-fs-4 text-link">
                                                    <div className="pro-btn-text">{pageData?.channels?.buttonname}</div>
                                                    <MdKeyboardArrowRight />
                                                </div>
                                            </div>
                                        </a>
            ...
            {pageData?.stats && (
                <div className="section bg-light stats-sec align-items-center my-80">
                    <div className="container h-100 d-flex flex-wrap text-lg-center text-start  align-items-center">
                        {pageData?.stats?.content.length > 0 &&
                            pageData?.stats?.content.map((stat, index) => {
                                return (
                                    <div className="col-12 col-sm-6 col-lg-3 d-flex flex-column  gap-2 py-3 py-sm-5">
                                        <h2 className="c-fs-1 c-fw-b">{stat?.count}</h2>
                                        <h3 className="c-fs-3">
                                            {stat?.name}
                                            <br /> {pageData?.channels?.each}
                                        </h3>
                                    </div>
                                );
                            })}
