...
The long code SMS service uses a ten-digit phone number, and is more relevant for businesses that intend to have continuous interaction with their customers. The [long code SMS service](https://msg91.com/in/sms/) is used for one-to-mean communication, and can only send one message per second.  
...
* Alert notifications:  
**Long code SMS** is used to send responsive messages to users. Message notifications that need to be communicated upon the completion of an action or sent as an alert, due to a trigger, use a [long code SMS service](https://msg91.com/in/sms/).
...
Businesses such as clinics, salons can use [long code SMS](https://msg91.com/in/sms/) to send appointment reminders to its clientele. It can also be used by commercial setups to send order confirmation details on a real-time basis.
